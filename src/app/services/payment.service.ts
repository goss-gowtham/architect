...
export class PaymentService {
  ...
  initiatePayUPayment(card: CardDTO): Observable<void> {
    ...
    const successUrl = `${window.location.origin}/architect/clients?status=success&projectId=${card.projectId}&userId=${card.username}`;
    const failureUrl = `${window.location.origin}/architect/clients?status=failure`;
    const hashString = `${this.key}|${transactionId}|${card.pay}|${card.project}|${card.username}|user@example.com|||||||||||${this.salt}`;
    ...
    return this.generateHash(hashString).pipe(
      switchMap(hash => {
        ...
        return this.http.post<void>(baseUrl, body.toString(), { headers });
      })
    );
  }
